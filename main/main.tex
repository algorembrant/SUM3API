...
\lstdefinelanguage{Rust}{
  keywords={fn, let, mut, loop, match, if, else, while, for, in, return, struct, enum, impl, use, pub, mod, crate, super, self, Self, true, false, async, await, move, type, trait, where, unsafe},
  keywordstyle=\color{magenta},
  ndkeywords={u8, u16, u32, u64, i8, i16, i32, i64, f32, f64, usize, isize, bool, str, String, Vec, Option, Result, Some, None, Ok, Err, Box},
...
\begin{figure}[H]
    ...
    \caption{\hyperref[sec:fig1_text_ref]{SUM3API System Framework}}
...
The official MetaTrader 5 Python package, introduced by MetaQuotes, enables Python scripts to connect to running MT5 terminals for data retrieval and order submission \cite{metaquotes_python2024}. While this integration expanded MT5's accessibility to the Python data science ecosystem, the package operates through an inter-process communication mechanism that typically relies on polling or synchronous requests, which may present challenges for use cases requiring microsecond-resolution real-time tick streaming compared to asynchronous push-based models.
...
The main toolchain included Git, Rust (with cargo), Visual Studio C++ Build Tools, MetaTrader 5, and MetaEditor 5.\label{ref:a2}\textsuperscript{\hyperref[sec:a2]{A2}} 
...
This separation of concerns aligns messaging patterns with data characteristics: high-frequency, unidirectional tick data employs the fire-and-forget semantics of PUB/SUB, while order execution requiring acknowledgment uses synchronous REQ/REP communication. \\[0.2em]

The \phantomsection\label{sec:fig1_text_ref}\hyperref[fig:placeholder]{Fig. 1} illustrates the complete system framework and data flow. The architecture is divided into three layers: the MetaTrader 5 layer, the ZeroMQ Transport layer, and the Rust Application layer. In the MetaTrader 5 layer, the \texttt{Zmq.mqh} wrapper facilitates communication for the \texttt{ZmqPublisher.mq5} Expert Advisor, which utilizes \texttt{libzmq.dll} and \texttt{libsodium.dll}. This layer connects to the ZeroMQ Transport layer via two sockets: a PUB Socket on port 5555 for broadcasting Tick Data, and a REP Socket on port 5556 for handling Order Requests and Responses. The Rust Application layer mirrors this setup with a SubSocket for receiving ticks and a ReqSocket for sending orders, both feeding into the \texttt{egui}-based Trading GUI.
...
A second async task manages the REQ socket for order execution:\\[0.2em]
...
The main application thread runs an \texttt{egui}-based graphical interface, consuming tick data via MPSC channels and rendering real-time price charts, account information, and trade controls.\\[0.2em]

\subsection{Data Serialization}

All inter-process communication employed JSON serialization for message payloads. While JSON introduces serialization overhead compared to binary formats, it provides debugging transparency and simplifies cross-language compatibility. The tick data message structure includes the following example:\\[0.2em]
...
The MQL5 implementation totaled approximately 600 lines of code across four files: the \texttt{Zmq.mqh} wrapper (144 lines), the \texttt{ZmqPublisher.mq5} Expert Advisor (451 lines), and the \texttt{libsodium.dll \& libzmq.dll} DLL files\label{ref:a11}\textsuperscript{\hyperref[sec:a11]{A11}}.

\subsubsection{ZeroMQ Library Integration}

We integrated the native ZeroMQ library (\texttt{libzmq.dll}) through MQL5's \texttt{\#import} directive, which enables calling external DLL functions. The wrapper handles 64-bit pointer compatibility by using \texttt{long} type for context and socket handles:\\[1em]
...
\midrule
Live Tick Streaming\label{ref:a7}\textsuperscript{\hyperref[sec:a7]{A7}} & Real-time bid/ask data at tick-level granularity with timestamp \\
...
\begin{itemize}
    ...
    \item Accurate tick data transmission with no message loss under normal market conditions\phantomsection\label{ref:a3}\textsuperscript{\hyperref[sec:a3]{A3}}
...
All source code, complete end-to-end system architecture, MQL5-ZMQ \& RUST-ZMQ wrapper libraries for the SUM3API's system, documentations, and examples are available under an MIT open-source license at:
...
We installed the following applications: Git, Rust (with cargo), Visual Studio C++ Build Tools, MetaTrader 5, and MetaEditor 5. The development was conducted using Google Antigravity (any code editor with terminal access would suffice). 
...
This is particularly relevant for the features described in Table~\ref{tab:features}, specifically Account Monitoring and Trade Execution, where sensitive operations are performed without requiring credential transmission.

\subsection{\hyperref[ref:a11]{A11}: Library Dependencies}
\label{sec:a11}

The core dependencies for the MQL5 integration, \texttt{libzmq} and \texttt{libsodium}, are open-source libraries available for download from their official repositories. \texttt{libzmq} can be obtained from \cite{libzmq_dll} (repository: \cite{libzmq_github}), and \texttt{libsodium} is available at \cite{libsodium_dll} (repository: \cite{libsodium_github}). Users should ensure they download the versions compatible with their operating system architecture (typically 64-bit for modern MT5 installations).
