...
\author{
    \IEEEauthorblockN{Rembrant Oyangoren Albeos\textsuperscript{$\dagger$}}
    \IEEEauthorblockA{
        \textsuperscript{$\dagger$}\textit{See Author Information on Page~\pageref{sec:author_info}}
    }
}
...
\begin{abstract}
MetaTrader 5 (MT5), when connected to preferred exchanges or brokers, supports automated algorithmic trading via Expert Advisors (EAs) written in MetaQuotes Language (MQL5). While MetaQuotes LLC provides an official Python integration package, publicly documented methods for internally extracting and externally projecting MT5 financial data remain limited\textsuperscript{A9}. To address this gap, this study implements a novel approach that bridges MQL5 and Rust via ZeroMQ publisher--subscriber and request--reply bindings. This benchmark-based methodology enables quantitative researchers, feature engineers, and algorithmic traders to develop trading systems leveraging MT5 data feeds using Rust, thereby bypassing the limitations inherent to pure MQL5 Expert Advisors\textsuperscript{A6}. The methodology is validated through integration within a functional trading terminal application\textsuperscript{A1} demonstrating low-latency capabilities including: real-time account information monitoring (balance, equity, free and used margin), downloadable historical data requests (OHLC bars and raw tick data), downloadable forward data streaming (live tick recording)\textsuperscript{A7}, trade execution controls (market, limit, and stop orders with lot sizing and cancellation), messaging and notifications for debugging \& recent calls, and a live microsecond-resolution raw tick-level bid/ask price formation chart\textsuperscript{A3}. All resources are open-source and available on GitHub: \url{https://github.com/algorembrant/Rust-ZMQ-MT5}.
\end{abstract}
...
\begin{enumerate}
    ...
    \item To document the methodology with sufficient detail for reproducibility\textsuperscript{A2}
\end{enumerate}
...
The implementation was conducted on a 7-year old Windows 10 Pro 64-bit laptop with the following specifications:
...
The development toolchain included Git, Rust (with cargo), Visual Studio C++ Build Tools, MetaTrader 5, and MetaEditor 5\textsuperscript{A2}. The code was developed using Google Antigravity as the primary editor.
...
All metadata are fetched automatically, independent of the ticker or symbol attached to the Expert Advisor. For this implementation, the EA can be attached to any chart (e.g., XAUUSDc or BTCUSDc) without modifying the code\textsuperscript{A1}.
...
The MQL5 implementation totals approximately 600 lines of code across four files: the \texttt{Zmq.mqh} wrapper (144 lines), the \texttt{ZmqPublisher.mq5} Expert Advisor (451 lines), and the \texttt{libsodium.dll \& libzmq.dll} DLL files.
...
\begin{itemize}
    ...
    \item Live tick recording to CSV\textsuperscript{A7}
...
Live Recording & Record incoming tick stream to timestamped CSV files\textsuperscript{A7} \\
...
The system was validated through integration testing within a live MT5 environment connected to an Exness broker Standard-cent live trading account\textsuperscript{A1}. Validation criteria included:

\begin{itemize}
    \item Successful connection establishment between MQL5 and Rust components
    \item Accurate tick data transmission with no message loss under normal market conditions\textsuperscript{A3}
...
\begin{enumerate}
    \item \textbf{Language Independence}: While this implementation uses Rust, the ZeroMQ-based architecture supports any language with ZeroMQ bindings, including C++, Java, and Go.
...
\begin{enumerate}
    ...
    \item \textbf{Single Symbol Scope}: The current implementation operates on the chart symbol where the EA is attached; multi-symbol support would require architecture extensions\textsuperscript{A5}.
    ...
    \item \textbf{Data Persistence}: The software loses live data if closed; the database only stores ``requested'' information and not automatically---data cannot be recovered if connection is lost unless recording is manually toggled on\textsuperscript{A7}.
\end{enumerate}

\subsection{Practical Applications}

The methodology enables several practical applications\textsuperscript{A4}:
...
\begin{enumerate}
    ...
    \item Open-source implementation enabling reproducibility and extension\textsuperscript{A6}
\end{enumerate}
...
\begin{itemize}
    \item Multi-symbol support through symbol-keyed message routing\textsuperscript{A5}
    ...
    \item Docker containerization for simplified deployment\textsuperscript{A8}
\end{itemize}
...
The author is an independent researcher (non-affiliated with professionally known institutions). Developing this study from scratch to final distribution required considerable time, but with the help of xAI's Grok for searching for facts \cite{grok2024}, Google's Gemini Pro \cite{gemini2024}, and Anthropic's Claude \cite{claude2024} inside Google Antigravity, the author was able to complete this work. I thank all the mentioned companies; if it were not for their masterpiece, I would have a very hard time completing this work.

The author acknowledges the ZeroMQ community for providing the robust messaging library that forms the foundation of this work, and the Rust community for the excellent async ecosystem enabling this implementation. If it were not for their existence, the author would not have an idea of how to connect these three APIs.

The author thanks himself for having a goal to contribute to the research community rather than presenting a non-implementable approach or complicated methods; for answering one gap; for staying strong despite the unfamiliar challenges; and for the willingness to explore and grow again after backing down.
...
\begin{center}
\textbf{Rembrant Oyangoren Albeos}
\end{center}
...
\subsection{A1: Trading Account and Broker Information}

The author uses Exness as the broker with a Standard-cent live trading account. Since a cent-based account is used, all tradable assets have a ``c'' suffix (e.g., XAUUSDc for gold, BTCUSDc for Bitcoin, etc.). This implementation uses a live account, not a demo or paper trading account.

\subsection{A2: Development Environment Setup}

The author has installed the following applications: Git, Rust (with cargo), Visual Studio C++ Build Tools, MetaTrader 5, and MetaEditor 5. The development was conducted using Google Antigravity (any code editor with terminal access would suffice). The specific versions used are not critical to the implementation.

\subsection{A3: Tick Data Accuracy Note}

The current implementation has not been compared against other bid/ask formation approaches (dataframes, line chart formations). Therefore, the author cannot definitively claim no data loss or perfectly successful extraction. The microsecond-resolution raw bid/ask formation uses a non-fixed-time-interval x-axis; the formation depends on raw up-and-down tick movement count. Thus, the statement ``accurate tick data transmission with no message loss'' applies only ``under normal market conditions.''

\subsection{A4: Related Python Project}

The author has a dedicated Python project for fetching tradable assets (forex, crypto) from an Exness Standard-cent live account. The project is available at: \url{https://github.com/algorembrant/QRAT2025/blob/main/Python\%20Folder/Ticker\%20Quotes\%20Fetching/Quotes\%20of\%20all\%20tradable\%20assets\%20in\%20Exness.ipynb}

\subsection{A5: Multi-Symbol Support}

For the ``Single Symbol Scope'' limitation, there is a way to address this---the author has implemented it in previous projects where an EA attached to one symbol can trade different symbols. However, this functionality is kept out of this research study for simplicity. The code for multi-symbol support is not publicly available; interested parties may contact the author. The reason for keeping it private is that it implements an automated algorithmic trading bot with a diversification model. Documentation for that EA is available at: \url{https://github.com/algorembrant/QRAT2025/blob/main/Research\%20Papers/The\%203HTMOSDSDT.rp2}

\subsection{A6: Library Completeness}
...
\subsection{A7: Data Persistence Limitation}
...
\subsection{A8: Docker Containerization}

The author has been in software development since mid-November 2025 and has not yet learned Docker. Docker containerization is listed as a future work item.

\subsection{A9: Literature Search Methodology}
...
The author searched across the internet using nine leading AI platforms: Grok \cite{grok2024}, Gemini \cite{gemini2024}, Claude \cite{claude2024}, ChatGPT \cite{chatgpt2024}, Microsoft Copilot \cite{mscopilot2024}, Perplexity \cite{perplexity2024}, Baidu (Ernie Bot) \cite{ernie2024}, DeepSeek \cite{deepseek2024}, and Qwen \cite{qwen2024}. Some AI responses hallucinated, suggesting a combination of 2 APIs instead of strictly 3; these were later clarified and confirmed that no prior public projects have implemented this exact combination.
...
However, no prior work combines all three: MQL5, ZeroMQ, and Rust in a unified API bridge.
