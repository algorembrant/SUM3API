...
\begin{enumerate}
    ...
    \item \textbf{Single Symbol Scope}: The current implementation operated on the chart symbol where the EA was attached; multi-symbol support would require architecture extensions\label{ref:a5}\textsuperscript{\hyperref[sec:a5]{A5}} \cite{qrat2025_multisymbol_ea}.
...
\begin{itemize}
    ...
    \item \textbf{Separation of Concerns}: Authentication is handled by MT5's native security mechanisms, while the external application focuses solely on data consumption and command issuance
